...
% (C) Laurens van der Maaten, Delft University of Technology
	...
	M = M(:,ind(1:no_dims));
    newlambda = lambda(1:no_dims);
	
	% Apply mapping on the data
    if ~(size(X, 2) < size(X, 1))
        M = bsxfun(@times, X' * M, (1 ./ sqrt(size(X, 1) .* newlambda))');     % normalize in order to get eigenvectors of covariance matrix
    end
    mappedX = X * M;
